...
class SimpleCNN(nn.Module):
    def __init__(self, scale_factor=4, num_in_ch=3, num_feat=64, p=0.25):
        super(SimpleCNN, self).__init__()

        self.conv = BSConvU

        self.fea_conv = self.conv(num_in_ch, num_feat, kernel_size=3)
        # self.fea_conv = BSConvU(num_in_ch, num_feat, kernel_size=3)
        ...
        self.B3 = ESDB(in_channels=num_feat // 2, out_channels=num_feat // 2, conv=self.conv, p=p)
