...
def resize_worker(img_file, sizes, resample, lmdb_save=False):
    ...
    img_name = extract_numbers(img_name)
    return img_name, out
...
def prepare(img_path, out_path, n_worker, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False):
    ...
    if not lmdb_save:
        ...
        env = lmdb.open(out_path, map_size=1024 ** 3, readahead=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', '-o', type=str,
                        default='../dataset/DF2K')

    parser.add_argument('--size', type=str, default='50,200')
