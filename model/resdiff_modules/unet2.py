...
class CCM(nn.Module):
    def __init__(self, dim, growth_rate=4.0):
        super().__init__()
        hidden_dim = int(dim * growth_rate)  # 表示的是CCM的中间channel数
...
class Multi_Att(nn.Module):
    def __init__(self, ch_in, reduction=2, dim = 6, ffn_scale=4.0):
        super(Multi_Att, self).__init__()
        self.init_conv = nn.Conv2d(ch_in, ch_in * 2, 3, 1, 1)
        self.res = ResSE(ch_in * 2, reduction)
        self.ccm = CCM(ch_in * 2, ffn_scale)  # CCM block
        self.safm = SAFM(ch_in * 4)  # SAFM block
        self.final_conv = nn.Conv2d(ch_in * 4, ch_in, 3, 1, 1)

        """
        把这边的通道数改一下
        在前面已经×2了  已经变成12了  现在再乘就已经变成24了
        """

    def forward(self, x):
        x = self.init_conv(x)
        x = torch.cat((self.res(x), self.ccm(x)), dim=1)
        x = self.safm(x)
        x = self.final_conv(x)
        return x
...
class FD_Info_Spliter(nn.Module):
    def __init__(self, dim, in_channels, out_channels, image_size):
        ...
        self.sigma_resSE = Multi_Att(in_channels * 2)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.HF_guided_resSE = ResSE(in_channels * 2)
        self.channel_transform = nn.Conv2d(6, 3, 1)

        # FD_Info_Spliter(dim=32, in_channels=3, out_channels=3, image_size=image_size)
...
class HF_guided_CA(nn.Module):
    def __init__(self, in_channel, norm_groups=32):
        ...
        self.q = nn.Conv2d(3, in_channel, 1, bias=False).to('cuda')
        self.kv = nn.Conv2d(in_channel, in_channel * 2, 1, bias=False).to('cuda')
        self.out = nn.Conv2d(in_channel, in_channel, 1).to('cuda')

    def forward(self, input, mff): # input 是UNet中间tensor，quary是DWT的特征
        batch, channel, height, width = input.shape
        head_dim = channel
        ...
        mff = self.q(mff).unsqueeze(1) # quary先经过卷积  在quary的dim=1上添加一个维度

        attn = torch.einsum(
            "bnchw, bncyx -> bnhwyx", mff, key
        ).contiguous() / math.sqrt(channel)
...
class UNet(nn.Module):
    def __init__(
        ...
        self.mffblock = MFFB(input_dim=1, k = 32)
        K = 32
        self.mf_feature_conv = nn.Sequential(
            nn.Conv2d(K, 3, kernel_size=3, stride=2, padding=1),
        ...
        # for i in range(self.J):
        #     self.mff_list.append(self.mffblock(input_dim = 1, K = inner_channel * (2 ** i)))
        # self.mff_list = nn.ModuleList(self.mff_list)
    ...
    def forward(self, x, time):
        ...
        mf_feature = self.mffblock(dwt_x)

        mff_img_list = []
        for layer in self.mf_feature_conv:
            mf_feature = layer(mf_feature)
            mff_img_list.append(mf_feature)

        # Performing time-step embedding
        t = self.noise_level_mlp(time) if exists(
            self.noise_level_mlp) else None # t:[1,1,64]

        x = self.fd_spliter(x, t) # 将输入和时间步送入FD spliter 是UNet初始条件的处理 输出得到x:[1,15,128,128]
