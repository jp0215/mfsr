...
class FD_Info_Spliter(nn.Module):
    ...
    def forward(self, x, noise_embed):
        ...
        # return torch.cat([x, cnn_x, x_lf_feature, x_hf_feature], dim=1)
...
class HF_guided_CA(nn.Module):
    def __init__(self,image_size, in_channel, norm_groups=32, growth_rate=2.0):
        super().__init__()
        hidden_dim = int(in_channel * growth_rate)

        self.norm = nn.GroupNorm(norm_groups, in_channel).to('cuda')
        self.q = nn.Conv2d(3, in_channel, 1, bias=False)

        self.ff_parser_attn_map = nn.Parameter(torch.ones(in_channel, image_size, image_size))
        ...
        self.ccm = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(hidden_dim, in_channel, 1, 1, 0)
        )
    ...
    def forward(self, input, quary): # input 是UNet中间tensor，quary是DWT的特征
        ...
        # ff_parser_attn_map = self.ffpaser_conv(norm)

        dtype = norm.dtype
        norm = fft2(norm)
        norm = norm * self.ff_parser_attn_map
